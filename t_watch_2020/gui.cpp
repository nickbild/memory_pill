...
#define RTC_TIME_ZONE   "EST5EDT,M3.2.0,M11.1.0"
...
void setupGui()
{
    ...
    //Create wallpaper
    void *images[] = {(void *) &bg, (void *) &bg1, (void *) &bg2, (void *) &bg3, (void *) &bg4 };
...
//    lv_img_set_src(img_bin, images[r]);
    ...
    timeStyle.text.color = LV_COLOR_BLACK;
...
static void updateTime()
{
    ...
    strftime(buf, sizeof(buf), "%I:%M%p", &info);
...
public:
    ...
    {
        lv_obj_t *btn = lv_list_add_btn(_listCont, NULL, txt);
...
static List *list_light = nullptr; 
...
void light_list_add(const char *ssid)
{
    if (list_light == nullptr) {
        list_light = new List;
        list_light->create();
        list_light->align(bar.self(), LV_ALIGN_OUT_BOTTOM_MID);
        list_light->setListCb(wifi_list_cb);
    }
    list_light->add(ssid);
}
...
static void light_event_cb()
{
    ...
    int start = line_str.indexOf("\"",2);
    int endl = line_str.indexOf("\"", start+1);
    line_str = line_str.substring(start+1, endl);
